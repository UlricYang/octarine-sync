 ...
 * }
 */
func levelOrder(root *TreeNode) [][]int { // 主函数：接收根节点指针，返回层序遍历结果二维切片
...
- `queue := []*TreeNode{root}`：初始化队列并压入根节点，启动BFS遍历
...
- `const queue: TreeNode[] = [root];`：TypeScript的类型注解确保队列中元素均为TreeNode类型
...
- `queue.push_back(Rc::clone(child))`：将子节点加入队尾
...
| ---- | ------ | --------------------- | ---------------------- |
| 1    | [3]    | []                    | 加入根节点3            |
| 2    | [9,20] | \[[3]\]               | 处理3，加入子节点9,20  |
| 3    | [20]   | \[[3],[9]\]           | 处理9，无子节点        |
| 4    | [15,7] | \[[3],[9,20]\]        | 处理20，加入子节点15,7 |
| 5    | [7]    | \[[3],[9,20],[15]\]   | 处理15，无子节点       |
| 6    | []     | \[[3],[9,20],[15,7]\] | 处理7，无子节点，结束  |

✅ 最终输出：`[[3],[9,20],[15,7]]`，符合预期
...
| `levelOrder`        | 树对象                                       | `[[3],[9,20],[15,7]]` |
| `reflect.DeepEqual` | `[[3],[9,20],[15,7]]`, `[[3],[9,20],[15,7]]` | `true`                |
...
综上，该算法在时间和空间上均为理论最优，无进一步优化空间。O(n)时间与O(w)空间是二叉树层序遍历的自然下界
...
掌握 "队列+BFS+levelSize控制" 模式，不仅解决了本题，更构建了一个可迁移、可扩展的层序遍历思维框架，是解决所有层级处理问题的关键
