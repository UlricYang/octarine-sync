...
本题的最优解是 基于显式栈的迭代式前序遍历，其核心优势在于完全模拟递归逻辑、避免递归栈溢出风险、逻辑清晰且易于调试，并在工程实践中展现出卓越的稳定性与可控性

### 支撑论点：

#### A. 为什么迭代法是工程实践中的最优选择？

- 前序遍历的顺序是“根 → 左 → 右”，这使得使用栈模拟变得直观：
  1. 访问当前节点（根）；
  1. 将右子节点压入栈（后访问）；
  1. 将左子节点压入栈（先访问）；
  1. 重复直到栈空
- 这一策略充分利用了栈的后进先出（LIFO） 特性，完美契合前序遍历的需求
- 相较于递归，迭代法通过显式栈控制流程，避免了系统栈溢出的风险，尤其适合深度较大的树
- 代码结构清晰，易于扩展为非递归的通用树遍历模板
...
- 仅需前序遍历，不要求保留原始树结构（Morris 遍历会临时修改树）
...
- 代码结构清晰，便于调试、测试和性能分析
- 适合嵌入到生产级系统中，如表达式解析、语法树遍历、前缀表达式构建等
 ...
 * }
 */
func preorderTraversal(root *TreeNode) []int { // 主函数：接收根节点指针，返回前序遍历结果切片
	if root == nil { // 判断根节点是否为空，是则直接返回空切片
		return []int{}
	}

	stack := []*TreeNode{root} // 初始化栈，压入根节点
	result := []int{}          // 初始化结果切片
...
- `stack := []*TreeNode{root}`：初始化栈并压入根节点，启动前序遍历
- `result := []int{}`：用于存储遍历结果
- 循环不变量：每次循环开始时，栈顶元素为下一个待访问的节点
- 访问顺序控制：
  - 先压入右子节点，再压入左子节点；
  - 由于栈是后进先出，左子节点会先被弹出，从而保证“根→左→右”的访问顺序
...
- `while stack:`：Python 的布尔逻辑简洁，非空列表为 `True`
- 压栈顺序与 Go 一致，确保“根→左→右”的访问顺序
...
- `const stack: TreeNode[] = [root];`：TypeScript 的类型注解确保栈中元素均为 `TreeNode` 类型
- `stack.pop()!`：由于循环条件保证栈非空，使用 `!` 非空断言避免类型系统报错
- `if (current.right)` 和 `if (current.left)`：显式判断子节点是否存在
...
- `stack.push(Rc::clone(right))` 和 `stack.push(Rc::clone(left))`：
...
| ---- | ----- | ------- | ----------------------------------------- |
| 1    | [1]   | []      | 压入根节点 1                              |
| 2    | [2]   | [1]     | 弹出 1，压入右子节点 2（左子节点为 null） |
| 3    | [2,3] | [1,2]   | 弹出 2，压入右子节点 null，压入左子节点 3 |
| 4    | [2]   | [1,2,3] | 弹出 3，无子节点                          |
| 5    | []    | [1,2,3] | 弹出 2，无更多节点，结束                  |
...
综上，该算法在时间和空间上均为理论最优（除 Morris 遍历外），无进一步优化空间。O(n) 时间与 O(h) 空间是二叉树前序遍历的自然下界

## 7. 技巧归纳与模式抽象

### 核心结论：

本题的本质是 “通用前序遍历迭代模板”，其核心在于用显式栈模拟递归调用栈、通过“先压右子节点，再压左子节点”控制访问顺序，这一模式可泛化至所有二叉树的深度优先遍历（前序、中序、后序）
...
| 题号 | 题目名称         | 核心思想                              | 匹配模式 |
| ---- | ---------------- | ------------------------------------- | -------- |
| 94   | 二叉树中序遍历   | 栈 + 一路向左 + 弹出访问 + 转向右子树 | 模式复用 |
| 145  | 二叉树后序遍历   | 双栈法或标记法                        | 模式复用 |
| 589  | N叉树前序遍历    | 栈 + 反向压入子节点                   | 模式复用 |
| 257  | 二叉树的所有路径 | 前序遍历 + 路径记录                   | 模式应用 |
| 112  | 路径总和         | 前序遍历 + 累积路径值                 | 模式应用 |

> 本题是 “二叉树 DFS 迭代模板” 的标准实现，是后续学习中序、后序遍历的基础

#### C. 模式的泛化与应用场景拓展

- 表达式解析：前缀表达式（+ a b）对应前序遍历
- 语法树遍历：编译器中遍历抽象语法树（AST）
- 文件系统目录遍历：按名称排序的目录结构
- UI 组件树渲染：React/Vue 的组件树前序遍历

#### D. 工业界实际应用案例分析

- SQL 查询优化器：遍历查询计划树，生成最优执行顺序
- 前端框架模板编译：Vue/React 的组件树前序遍历
- 自动化测试框架：遍历测试用例树结构

#### E. 算法深入解析

- 最优子结构：对一棵二叉树的前序遍历，等于“根” + “左子树前序” + “右子树前序”
- 状态空间压缩：不需要保存“是否访问过子节点”的状态，因为栈结构天然记录了路径
- 边界处理完备性：空树、单节点、单层多子节点、深度链表均被正确处理
- 可扩展性：只需修改压入顺序，即可实现中序/后序遍历

### 总结：

掌握 “显式栈 + 先压右子节点再压左子节点” 模式，不仅解决了本题，更构建了一个可迁移、可扩展的二叉树遍历思维框架，是解决所有深度优先遍历问题的关键
...
##### Q1: 为什么先压入右子节点，再压入左子节点？

→ 标准回答：栈是后进先出，先压入右子节点，后压入左子节点，弹出顺序就是“左 → 右”，符合前序遍历的“根 → 左 → 右”顺序
→ 加分回答：这是迭代法模拟递归的核心技巧。递归中左子树会先被调用，迭代法通过调整压入顺序实现同样的效果。这体现了“控制流可编程”的思想

##### Q2: 如果题目要求中序遍历呢？如何修改？

→ 标准回答：使用两个 while 循环，一路向左压栈，弹出访问后转向右子树
→ 加分回答：更优方式是：内层循环“一路向左”压栈，外层循环弹出访问并转向右子树。这是中序遍历的标准迭代模板，与前序遍历的“直接压栈”形成对比
...
→ 加分回答：前序遍历是“深度优先”，意味着“先访问子树”。队列无法支持这种“递归式深入”，它只会逐层展开。这是 DFS 与 BFS 的本质区别
...
掌握本题的关键在于牢记 “显式栈模拟递归”、“先压右子节点，再压左子节点” 和 “前序遍历 = 根 → 左 → 右”，同时避免 混淆中序/后序遍历，最终形成可复用的二叉树遍历模板
...
| 压入顺序错误  | 先压左子节点，再压右子节点 | 牢记：先压右，再压左，确保弹出顺序为“左→右”                 |
