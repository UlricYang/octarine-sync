...
理解和编写递归算法最有效的方法是从「树」的视角去理解，「树」结构是一个非常有效的数据结构。把问题抽象成树结构，然后用代码去遍历这棵树，就是递归的本质
编写递归算法只可能有两种思维模式，都尝试套用一下，必然有一种能写出来：

- 「遍历」的思维模式 ~ 斐波那契数列 ~ 需要一个无返回值的遍历函数，在遍历的过程中收集结果
- 「分解问题」的思维模式 ~ 全排列 ~ 递归函数一定要有一个清晰的定义，说明这个函数参数的含义是什么，返回什么结果，才能利用这个定义来计算子问题，反推原问题的解
