# LeetCode 106 - 从中序与后序遍历序列构造二叉树

## 1. 题目描述

根据一棵树的中序遍历与后序遍历构造二叉树

注意:
你可以假设树中没有重复的元素

### 示例

```
输入:
中序遍历 inorder = [9,3,15,20,7]
后序遍历 postorder = [9,15,7,20,3]

输出:
    3
   / \
  9  20
    /  \
   15   7
```

### 约束条件

- `1 <= inorder.length <= 3000`
- `postorder.length == inorder.length`
- `-3000 <= inorder[i], postorder[i] <= 3000`
- `inorder` 和 `postorder` 均无重复元素
- `postorder` 中每一个值都在 `inorder` 中
- `inorder` 保证是树的中序遍历
- `postorder` 保证是树的后序遍历

## 2. 解法分析：递归划分法

### 核心结论

本题的最优解是递归划分法，其核心优势在于：

1. 利用后序和中序遍历的结构特性 - 后序遍历的最后一个元素是根节点，中序遍历可以划分左右子树
1. 实现逻辑高度自洽 - 每次递归处理一个子问题，清晰直观
1. 时间复杂度最优 - 通过哈希优化可达到O(n)时间复杂度
1. 直接映射到二叉树构造过程 - 符合树的递归定义

### 支撑论点

#### A. 为什么递归划分法是最优解？

1. 后序遍历特性：最后一个元素一定是当前子树的根节点
1. 中序遍历特性：根节点将数组分为左子树(左侧)和右子树(右侧)
1. 组合威力：后序提供"根是谁"，中序提供"根的左右子树范围"，两者结合可唯一确定一棵二叉树
1. 递归划分：每次取后序末元素作为根，在中序中定位其位置，分割出左/右子树区间，递归构建
1. 无冗余信息：仅用两个遍历序列，不依赖额外数据结构(哈希表虽提升效率但非本质)

#### B. 与其他算法的对比分析

| 方法               | 是否可行 | 时间复杂度 | 空间复杂度 | 实现难度 | 特点                 |
| ------------------ | -------- | ---------- | ---------- | -------- | -------------------- |
| 递归划分法(本解)   | ✅       | O(n)       | O(n)       | 中       | 面试首选，逻辑清晰   |
| 哈希优化递归法     | ✅       | O(n)       | O(n)       | 中       | 效率更高，引入哈希表 |
| 迭代栈法           | ✅       | O(n)       | O(n)       | 高       | 代码复杂，难调试     |
| 前序+中序构造      | ✅       | O(n)       | O(n)       | 中       | 同类题，根在前序首位 |
| 暴力递归(每次查找) | ✅       | O(n²)      | O(n)       | 低       | 会超时，不可取       |

> 哈希优化本质是预处理优化，不改变算法范式。本题核心是"递归划分"思想

#### C. 适用条件与边界

1. 必须是二叉树：N叉树无唯一中序定义
1. 节点值唯一：若重复，中序中无法唯一定位根
1. 序列完整一致：两序列必须来自同一棵树
1. 无空节点标记：序列中不含null，不能直接用于序列化

#### D. 工程实践考量

1. 递归深度：最坏O(n)，需考虑栈溢出(可用迭代模拟)
1. 参数传递：使用索引而非切片，避免数组复制
1. 可读性优先：面试中优先清晰递归版本
1. 调试友好：每层递归对应一个子树，可打印区间验证

### 总结

递归划分法是本题在理论正确性、思维清晰度和工程表达上的最优平衡点

## 3. 多语言实现与深度解析

### 核心结论

通过四种语言实现，验证算法通用性，并洞察不同语言在递归、索引管理和内存效率上的差异

### A. Go 🐹 实现(基础版)

```go
/
 * Definition for a binary tree node.
 * type TreeNode struct {
 *     Val int
 *     Left *TreeNode
 *     Right *TreeNode
 * }
 */
func buildTree(inorder []int, postorder []int) *TreeNode {
    if len(postorder) == 0 {
        return nil
    }

    // 后序最后一个元素为根节点
    rootVal := postorder[len(postorder)-1]
    root := &TreeNode{Val: rootVal}

    // 在中序中查找根节点位置
    rootIdx := 0
    for i, val := range inorder {
        if val == rootVal {
            rootIdx = i
            break
        }
    }

    // 划分左右子树
    leftSize := rootIdx
    rightSize := len(inorder) - 1 - rootIdx

    // 递归构建左右子树
    root.Left = buildTree(inorder[:rootIdx], postorder[:leftSize])
    root.Right = buildTree(inorder[rootIdx+1:], postorder[leftSize:len(postorder)-1])

    return root
}
```

#### 算法深入解析

1. 递归基线：`if len(postorder) == 0` - 空序列返回nil
1. 根节点确定：`postorder[len(postorder)-1]` - 后序末元素必为根
1. 根位置查找：线性扫描中序数组 - O(n)时间
1. 子树划分：
   - `leftSize = rootIdx` - 左子树节点数
   - `rightSize = len(inorder)-1-rootIdx` - 右子树节点数
1. 递归构建：
   - 左子树：中序[0:rootIdx], 后序[0:leftSize]
   - 右子树：中序[rootIdx+1:], 后序[leftSize:len-1]

设计动机：

- 完全基于递归划分，每层处理一个子树
- 切片复制导致O(n²)时间，但逻辑清晰
- 体现"后序定根，中序分左右"的核心思想

### B. Go 🐹 实现(优化版)

```go
func buildTree(inorder []int, postorder []int) *TreeNode {
    // 预处理中序索引
    idxMap := make(map[int]int)
    for i, v := range inorder {
        idxMap[v] = i
    }

    var build func(int, int, int, int) *TreeNode
    build = func(inStart, inEnd, postStart, postEnd int) *TreeNode {
        if postStart >= postEnd {
            return nil
        }

        rootVal := postorder[postEnd-1]
        root := &TreeNode{Val: rootVal}
        rootIdx := idxMap[rootVal]

        leftSize := rootIdx - inStart
        root.Left = build(inStart, rootIdx, postStart, postStart+leftSize)
        root.Right = build(rootIdx+1, inEnd, postStart+leftSize, postEnd-1)

        return root
    }

    return build(0, len(inorder), 0, len(postorder))
}
```

#### 算法深入解析

1. 哈希预处理：`idxMap`存储中序值→索引，O(1)查找
1. 索引传递：使用区间参数避免切片复制
1. 根节点处理：`postorder[postEnd-1]`获取根值
1. 子树划分：
   - `leftSize = rootIdx - inStart` - 左子树大小
   - 左子树区间：中序\[inStart, rootIdx), 后序\[postStart, postStart+leftSize)
   - 右子树区间：中序\[rootIdx+1, inEnd), 后序\[postStart+leftSize, postEnd-1)

优化点：

- 时间复杂度降为O(n)
- 空间复杂度O(n)用于哈希表
- 无数组复制，内存效率高

### C. Python 🐍 实现

```python
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
#         self.val = val
#         self.left = left
#         self.right = right

class Solution:
    def buildTree(self, inorder: List[int], postorder: List[int]) -> Optional[TreeNode]:
        # 预处理中序索引
        idx_map = {val: idx for idx, val in enumerate(inorder)}

        def helper(in_start, in_end, post_start, post_end):
            if post_start >= post_end:
                return None

            root_val = postorder[post_end-1]
            root = TreeNode(root_val)
            root_idx = idx_map[root_val]

            left_size = root_idx - in_start
            root.left = helper(in_start, root_idx, post_start, post_start+left_size)
            root.right = helper(root_idx+1, in_end, post_start+left_size, post_end-1)

            return root

        return helper(0, len(inorder), 0, len(postorder))
```

#### 算法深入解析

1. 字典预处理：`idx_map`快速查找根位置
1. 闭包函数：`helper`使用外部`idx_map`
1. 区间处理：与Go优化版本逻辑一致
1. Python特性：
   - 字典推导式简洁高效
   - 闭包访问外部变量
   - 类型提示增强可读性

### D. TypeScript 🟦 实现

```typescript
/
 * Definition for a binary tree node.
 * class TreeNode {
 *     val: number
 *     left: TreeNode | null
 *     right: TreeNode | null
 *     constructor(val?: number, left?: TreeNode | null, right?: TreeNode | null) {
 *         this.val = (val===undefined ? 0 : val)
 *         this.left = (left===undefined ? null : left)
 *         this.right = (right===undefined ? null : right)
 *     }
 * }
 */

function buildTree(inorder: number[], postorder: number[]): TreeNode | null {
    const idxMap = new Map<number, number>();
    inorder.forEach((val, idx) => idxMap.set(val, idx));

    const build = (inStart: number, inEnd: number, postStart: number, postEnd: number): TreeNode | null => {
        if (postStart >= postEnd) return null;

        const rootVal = postorder[postEnd - 1];
        const root = new TreeNode(rootVal);
        const rootIdx = idxMap.get(rootVal)!;

        const leftSize = rootIdx - inStart;
        root.left = build(inStart, rootIdx, postStart, postStart + leftSize);
        root.right = build(rootIdx + 1, inEnd, postStart + leftSize, postEnd - 1);

        return root;
    };

    return build(0, inorder.length, 0, postorder.length);
}
```

#### 算法深入解析

1. Map预处理：`idxMap`存储值→索引
1. 箭头函数：`build`使用外部`idxMap`
1. 类型安全：
   - `Map<number, number>`明确类型
   - `!`断言非空(题目保证存在)
1. TypeScript特性：
   - 类型注解增强可维护性
   - 箭头函数保持this上下文

### E. Rust 🦀 实现

```rust
// Definition for a binary tree node.
// #[derive(Debug, PartialEq, Eq)]
// pub struct TreeNode {
//   pub val: i32,
//   pub left: Option<Box<TreeNode>>,
//   pub right: Option<Box<TreeNode>>,
// }
//
// impl TreeNode {
//   #[inline]
//   pub fn new(val: i32) -> Self {
//     TreeNode {
//       val,
//       left: None,
//       right: None,
//     }
//   }
// }

use std::collections::HashMap;

impl Solution {
    pub fn build_tree(inorder: Vec<i32>, postorder: Vec<i32>) -> Option<Box<TreeNode>> {
        let idx_map: HashMap<i32, usize> = inorder.iter()
            .enumerate()
            .map(|(i, &v)| (v, i))
            .collect();

        fn build(
            inorder: &[i32],
            postorder: &[i32],
            in_start: usize,
            in_end: usize,
            post_start: usize,
            post_end: usize,
            idx_map: &HashMap<i32, usize>
        ) -> Option<Box<TreeNode>> {
            if post_start >= post_end {
                return None;
            }

            let root_val = postorder[post_end - 1];
            let root_idx = idx_map[&root_val];
            let left_size = root_idx - in_start;

            let mut root = TreeNode::new(root_val);
            root.left = build(
                inorder,
                postorder,
                in_start,
                root_idx,
                post_start,
                post_start + left_size,
                idx_map
            );
            root.right = build(
                inorder,
                postorder,
                root_idx + 1,
                in_end,
                post_start + left_size,
                post_end - 1,
                idx_map
            );

            Some(Box::new(root))
        }

        build(
            &inorder,
            &postorder,
            0,
            inorder.len(),
            0,
            postorder.len(),
            &idx_map
        )
    }
}
```

#### 算法深入解析

1. HashMap预处理：`idx_map`存储值→索引
1. 切片引用：`&inorder`和`&postorder`避免所有权转移
1. 嵌套函数：`build`使用外部`idx_map`
1. Rust特性：
   - 所有权系统确保内存安全
   - 切片引用提高效率
   - 迭代器链式调用简洁高效

### F. 性能对比与语言特性分析

| 语言       | 时间复杂度 | 空间复杂度 | 实现方式     | 特点               |
| ---------- | ---------- | ---------- | ------------ | ------------------ |
| Go(基础)   | O(n²)      | O(n²)      | 切片复制     | 逻辑清晰，面试首选 |
| Go(优化)   | O(n)       | O(n)       | 索引传递     | 工业级实现         |
| Python     | O(n)       | O(n)       | 字典+闭包    | 简洁高效           |
| TypeScript | O(n)       | O(n)       | Map+箭头函数 | 类型安全           |
| Rust       | O(n)       | O(n)       | HashMap+切片 | 内存安全           |

## 4. 算法可视化与伪代码

### 伪代码

```
函数 buildTree(inorder, postorder):
    如果 postorder 为空:
        返回 null

    根值 = postorder 最后一个元素
    创建根节点

    在 inorder 中查找根值的索引 rootIdx

    左子树大小 = rootIdx
    右子树大小 = inorder 长度 - 1 - rootIdx

    左子树 = buildTree(
        inorder[0:rootIdx],
        postorder[0:leftSize]
    )

    右子树 = buildTree(
        inorder[rootIdx+1:],
        postorder[leftSize:len(postorder)-1]
    )

    返回 根节点
```

### Mermaid 流程图

```mermaid
graph TD
    %% --- 高对比度样式定义 ---
    classDef startEnd fill:#bbdefb,stroke:#1565c0,stroke-width:2px,color:#0d47a1
    classDef process fill:#fff9c4,stroke:#f57f17,stroke-width:2px,color:#1a237e
    classDef decision fill:#ffccbc,stroke:#d84315,stroke-width:2px,color:#4e0000
    classDef recursion fill:#c8e6c9,stroke:#1b5e20,stroke-width:2px,color:#003d00
    classDef partition fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,color:#01579b
    classDef error fill:#ffcdd2,stroke:#b71c1c,stroke-width:2px,color:#4a0000

    %% --- 主流程入口 ---
    A(开始<br>构建二叉树) --> B{后序数组为空?}
    class A,O startEnd
    class B decision

    %% --- 递归基 ---
    B -- 是 --> C[返回 null]
    class C recursion

    B -- 否 --> D{数组只有一个元素?}
    class D decision

    D -- 是 --> E[创建单节点树并返回]
    class E process

    %% --- 核心逻辑：确定左右子树根 ---
    D -- 否 --> F["前序第一个元素是根 '<b>rootVal</b>'"]
    class F process

    F --> G["前序第二个元素是左子树的根 '<b>leftRootVal</b>'"]
    class G process

    %% --- 关键分区：划分左右子树区间 ---
    subgraph 划分子树区间
        direction TB
        H(在后序数组中<br>查找 leftRootVal 的索引)
        class H process

        I[📌 左子树包含的节点数量<br><b>leftSize = leftRootIndex + 1</b>]
        class I process

        J[📍 左子树区间划分]
        class J partition

        J --> J1(前序: 1 ~ leftSize)
        J --> J2(后序: 0 ~ leftSize-1)

        K[📍 右子树区间划分]
        class K partition

        K --> K1(前序: leftSize+1 ~ 末尾)
        K --> K2(后序: leftSize ~ 末尾-1)
    end

    G --> 划分子树区间

    %% --- 递归构建子树 ---
    J1 & J2 --> L[⬅️ 递归构建左子树]
    K1 & K2 --> M[➡️ 递归构建右子树]
    class L,M recursion

    %% --- 合并与返回 ---
    L --> N[将左子树挂到根节点]
    M --> O[将右子树挂到根节点]
    class N,O process

    N & O --> P([返回根节点])
    class P startEnd

```

### 状态转移图(示例)

```mermaid
graph LR
    subgraph 执行过程: inorder=[9,3,15,20,7], postorder=[9,15,7,20,3]
        S1[第1层: root=3] --> S1a[inorder中3位置=1]
        S1a --> S1b[左子树: inorder[0:1]=[9], postorder[0:1]=[9]]
        S1b --> S1c[右子树: inorder[2:5]=[15,20,7], postorder[1:4]=[15,7,20]]

        S1c --> S2[第2层: root=20]
        S2 --> S2a[inorder中20位置=3]
        S2a --> S2b[左子树: inorder[2:3]=[15], postorder[1:2]=[15]]
        S2b --> S2c[右子树: inorder[4:5]=[7], postorder[2:3]=[7]]

        S2b --> S3[第3层: root=15, 无子树]
        S2c --> S4[第3层: root=7, 无子树]
        S1b --> S5[第2层: root=9, 无子树]
    end
```

## 5. 执行过程演示

### 示例: inorder=[9,3,15,20,7], postorder=[9,15,7,20,3]

#### 递归执行轨迹

| 层级 | inorder       | postorder     | 根值 | rootIdx | leftSize | rightSize | 左子树区间 | 右子树区间 |
| ---- | ------------- | ------------- | ---- | ------- | -------- | --------- | ---------- | ---------- |
| 1    | [9,3,15,20,7] | [9,15,7,20,3] | 3    | 1       | 1        | 3         | [9]        | [15,20,7]  |
| 2L   | [9]           | [9]           | 9    | 0       | 0        | 0         | []         | []         |
| 2R   | [15,20,7]     | [15,7,20]     | 20   | 1       | 1        | 1         | [15]       | [7]        |
| 3RL  | [15]          | [15]          | 15   | 0       | 0        | 0         | []         | []         |
| 3RR  | [7]           | [7]           | 7    | 0       | 0        | 0         | []         | []         |

#### 构建过程

1. 根=3，在inorder中位置1 → 左子树1个节点，右子树3个节点
1. 左子树: inorder=[9], postorder=[9] → 节点9
1. 右子树: inorder=[15,20,7], postorder=[15,7,20] → 根=20
1. 20的左子树: inorder=[15], postorder=[15] → 节点15
1. 20的右子树: inorder=[7], postorder=[7] → 节点7

最终树结构:

```
    3
   / \
  9  20
    /  \
   15   7
```

#### 可执行测试代码(Go)

```go
package main

import "fmt"

type TreeNode struct {
    Val   int
    Left  *TreeNode
    Right *TreeNode
}

func buildTree(inorder []int, postorder []int) *TreeNode {
    if len(postorder) == 0 {
        return nil
    }

    rootVal := postorder[len(postorder)-1]
    root := &TreeNode{Val: rootVal}

    rootIdx := 0
    for i, val := range inorder {
        if val == rootVal {
            rootIdx = i
            break
        }
    }

    leftSize := rootIdx
    rightSize := len(inorder) - 1 - rootIdx

    root.Left = buildTree(inorder[:rootIdx], postorder[:leftSize])
    root.Right = buildTree(inorder[rootIdx+1:], postorder[leftSize:len(postorder)-1])

    return root
}

// 层序遍历辅助函数
func levelOrder(root *TreeNode) []interface{} {
    if root == nil {
        return []interface{}{}
    }
    queue := []*TreeNode{root}
    result := []interface{}{}
    for len(queue) > 0 {
        node := queue[0]
        queue = queue[1:]
        if node == nil {
            result = append(result, nil)
        } else {
            result = append(result, node.Val)
            queue = append(queue, node.Left, node.Right)
        }
    }
    // 去除末尾nil
    for len(result) > 0 && result[len(result)-1] == nil {
        result = result[:len(result)-1]
    }
    return result
}

func main() {
    // Test Case 1
    inorder1 := []int{9, 3, 15, 20, 7}
    postorder1 := []int{9, 15, 7, 20, 3}
    tree1 := buildTree(inorder1, postorder1)
    fmt.Printf("Test Case 1: %v\n", levelOrder(tree1)) // [3,9,20,null,null,15,7]

    // Test Case 2
    inorder2 := []int{-1}
    postorder2 := []int{-1}
    tree2 := buildTree(inorder2, postorder2)
    fmt.Printf("Test Case 2: %v\n", levelOrder(tree2)) // [-1]

    // Test Case 3
    inorder3 := []int{2, 1}
    postorder3 := []int{2, 1}
    tree3 := buildTree(inorder3, postorder3)
    fmt.Printf("Test Case 3: %v\n", levelOrder(tree3)) // [1,null,2]
}
```

## 6. 复杂度分析

### 核心结论

基础版本时间复杂度O(n²)，空间复杂度O(n²)；优化版本时间复杂度O(n)，空间复杂度O(n)

### 支撑论点

#### A. 时间复杂度(基础版)

- 每层递归需在中序数组中查找根位置，耗时O(k)
- 最坏情况(退化树):
  - 第一层: O(n)
  - 第二层: O(n-1)
  - 第三层: O(n-2)
  - ...
  - 总时间: n + (n-1) + ... + 1 = O(n²)

#### B. 空间复杂度(基础版)

- 每次递归复制子数组:
  - 第一层: O(n)
  - 第二层: O(n-1) + O(0)
  - 第三层: O(n-2) + O(1)
  - 总空间: O(n²)

#### C. 优化版本复杂度

- 时间复杂度: O(n)
  - 哈希表预处理: O(n)
  - 每个节点访问一次: O(n)
- 空间复杂度: O(n)
  - 哈希表: O(n)
  - 递归栈: O(h) ≤ O(n)

#### D. 常数因子分析

- 切片复制开销: 内存分配 + 数据拷贝
- 哈希查找开销: O(1)平均时间
- 递归调用开销: 函数栈帧

#### E. 性能瓶颈与优化

- 瓶颈: 重复遍历中序数组查找根位置
- 优化: 哈希表预处理索引映射
- 工程权衡:
  - 面试中先写基础版本，再问"能否优化?"
  - 优化版本为加分项，但核心思想不变

## 7. 技巧归纳与模式抽象

### 核心结论

本题属于"遍历序列重构树"模式，核心在于:

1. 利用遍历顺序的结构性信息
1. 通过根位置划分递归子问题
1. 依赖唯一性保证结构唯一

### 支撑论点

#### A. 模式本质

- 树是递归结构，遍历序列是其"投影"
- 后序: 提供"根是谁" → 构造顺序
- 中序: 提供"根的左右子树范围" → 空间划分
- 组合即唯一编码: 两种遍历共同构成树的"DNA"

#### B. 相似题目映射

| 题号 | 题目          | 核心思想      | 匹配模式   |
| ---- | ------------- | ------------- | ---------- |
| 106  | 本题          | 后序+中序重建 | 核心模式   |
| 105  | 前序+中序重建 | 前序根在首    | 模式变体   |
| 889  | 前序+后序重建 | 无法唯一重建  | 模式边界   |
| 652  | 寻找重复子树  | 子树序列化    | 序列化模式 |

#### C. 模式泛化

- 语法树解析: 编译器根据表达式顺序构造AST
- 数据库索引: B+树节点按顺序写入，可重建
- 网络协议: 树状配置结构通过序列化流还原

#### D. 工业应用

- SQL解析器: 将中缀表达式解析为语法树
- JSON Schema: 树形结构通过"字段顺序"重建
- 编译器前端: LR解析器本质就是递归下降

#### E. 算法深入解析

1. 唯一性证明:
   - 数学归纳法
   - 基础: 单节点唯一
   - 假设: k\<n节点唯一
   - 归纳: n节点树，根由后序确定，中序划分左右子树，左右子树唯一 → 整体唯一

1. 为何必须唯一值:
   - 重复值导致中序中多个相同节点，无法判断当前根

1. 为何必须后序+中序:
   - 后序提供根，中序提供结构划分
   - 前序+后序不能唯一重建(除非满二叉树)

## 8. 面试追问与回答策略

### 核心结论

面试追问考察:

1. 对树结构与遍历关系的理解
1. 复杂度权衡的工程思维
1. 从基础到优化的递进能力

### A. 基础追问

#### Q1: 为什么必须用后序和中序?只用后序行吗?

→ 标准: 不行，后序只能知道根和顺序，不知道左右边界
→ 加分: 后序如[9,15,7,20,3]可以是多种结构，必须结合中序的位置信息

#### Q2: 如果有重复元素，还能重建吗?

→ 标准: 不能，中序中多个相同值无法确定根
→ 加分: 可扩展为"节点ID+值"结构，或使用索引元组

#### Q3: 为什么不用哈希表一开始就优化?

→ 标准: 面试中优先写清晰版本，再优化
→ 加分: 基础版展示理解本质，优化版展示工程能力

#### Q4: 如果给的是前序和中序，怎么改?

→ 标准: 前序第一个元素是根，其余逻辑相同
→ 加分: 改postorder[-1]为preorder[0]，划分方式不变

### B. 高阶追问

#### Q1: 如何实现不复制数组的O(n)解法?

→ 标准: 用哈希表存中序索引，传递索引范围
→ 加分: 提供优化版代码，展示索引传递技巧

#### Q2: 为什么前序+后序不能唯一重建?

→ 标准: 无法确定左右子树分界点
→ 加分: 举例[1,2,3]有多种结构，除非满二叉树

#### Q3: 如何扩展为支持N叉树?

→ 标准: 需要额外信息如子节点个数
→ 加分: 后序改为"子节点1,子节点2,...,根,子节点数"，中序需自定义规则

#### Q4: 这个算法在编译器中如何应用?

→ 标准: 语法树构造
→ 加分: LR解析器用后序(token序列)和语法规则(类似中序)构造AST

## 9. 复习要点提炼

### 核心结论

掌握本题关键:

1. 牢记"后序定根，中序分左右"
1. 递归划分思维模式
1. 索引传递优化技巧
1. 避免混淆不同遍历顺序

### A. 关键记忆点(🌟)

| 记忆点   | 口诀                             |
| -------- | -------------------------------- |
| 构造核心 | 后序定根，中序分左右             |
| 划分依据 | 左子树大小=中序根左侧元素个数    |
| 递归入口 | 每次递归:后序末元素=当前根       |
| 优化关键 | 哈希表预存中序索引，O(1)查根位置 |
| 易混陷阱 | 前序根在首，后序根在尾           |

### B. 易错陷阱(⚠️)

| 错误类型     | 触发场景                      | 应对措施                        |
| ------------ | ----------------------------- | ------------------------------- |
| 混淆前后序   | 把前序末元素当根              | 记住:前序根在首，后序根在尾     |
| 索引计算错误 | 左子树大小=rootIdx+1          | 左子树大小=rootIdx(索引从0开始) |
| 忘记边界检查 | 未判断postorder为空           | 每次递归前必须检查              |
| 切片越界     | postorder[leftSize:len-1]超出 | 用索引传递避免切片              |

### C. 面试评分关键词(✅)

| 关键词     | 应用场景               |
| ---------- | ---------------------- |
| 后序定根   | 描述算法第一步         |
| 中序划分   | 描述子树分割依据       |
| 递归划分   | 命名算法范式           |
| 索引传递   | 描述优化方案           |
| 唯一性保证 | 解释为什么必须两个序列 |

### D. 复习建议(📚🚀)

```
核心模式: 遍历序列重建树
├── 必要条件: 两种遍历 + 节点唯一
├── 本质: 结构-序列双向映射
├── 方法: 递归划分
│   ├── 后序+中序 → 根在后，分在中
│   └── 前序+中序 → 根在前，分在中
├── 优化: 哈希预处理 → O(n)
└── 应用: 编译器AST、序列化重建
```

### E. 解题模板

```text
// 基础版
函数 buildTree(inorder, postorder):
    if postorder 为空: return null
    root_val = postorder[-1]
    root_idx = inorder.index(root_val)
    left_size = root_idx
    root = TreeNode(root_val)
    root.left = buildTree(inorder[:root_idx], postorder[:left_size])
    root.right = buildTree(inorder[root_idx+1:], postorder[left_size:-1])
    return root

// 优化版
函数 buildTree(inorder, postorder):
    map = {val: idx for idx, val in enumerate(inorder)}
    def helper(in_start, in_end, post_start, post_end):
        if post_start >= post_end: return null
        root_val = postorder[post_end-1]
        root_idx = map[root_val]
        left_size = root_idx - in_start
        root = TreeNode(root_val)
        root.left = helper(in_start, root_idx, post_start, post_start+left_size)
        root.right = helper(root_idx+1, in_end, post_start+left_size, post_end-1)
        return root
    return helper(0, len(inorder), 0, len(postorder))
```

### 总结

将上述要点融会贯通，即可在面试中快速准确解决此类问题，展现系统性思维能力。🎉
