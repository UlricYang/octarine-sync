...
class Solution3:
    ...
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        if not matrix or not matrix[0]:
            return False
        return self.searchHelper(matrix, target, 0, 0, len(matrix) - 1, len(matrix[0]) - 1)

    def searchHelper(self, matrix: List[List[int]], target: int,
                     top: int, left: int, bottom: int, right: int) -> bool:
        # 边界检查
        if top > bottom or left > right:
            return False

        # 如果目标值不在当前区域范围内，直接返回false
        ...
        if midVal == target:
            ...
            # 目标值在左上、右上或左下区域
            return (self.searchHelper(matrix, target, top, left, midRow, midCol) or
                    self.searchHelper(matrix, target, top, midCol + 1, midRow, right) or
                    self.searchHelper(matrix, target, midRow + 1, left, bottom, midCol))
        else:
            # 目标值在右上、左下或右下区域
            return (self.searchHelper(matrix, target, top, midCol + 1, midRow, right) or
                    self.searchHelper(matrix, target, midRow + 1, left, bottom, midCol) or
                    self.searchHelper(matrix, target, midRow + 1, midCol + 1, bottom, right))
```
...
// 二分搜索优化实现
func searchMatrixBinaryOptimized(matrix [][]int, target int) bool {
    fmt.Printf("=== 二分搜索优化 ===\n")
    fmt.Printf("目标矩阵 (%d×%d):\n", len(matrix), len(matrix[0]))
    printMatrix(matrix)
    fmt.Printf("目标值: %d\n", target)
    ...
    for i, row := range matrix {
        ...
        if row[0] <= target && target <= row[len(row)-1] {
            fmt.Printf("第%d行[%d,%d]包含目标值，进行二分搜索\n", i, row[0], row[len(row)-1])
...
func searchMatrixDivideConquer(matrix [][]int, target int) bool {
    ...
    result := searchHelper(matrix, target, 0, 0, len(matrix)-1, len(matrix[0])-1)
    fmt.Printf("分治法结果: %t\n", result)
    fmt.Printf("============\n\n")
    return result
}
...
// 递归辅助函数
func searchHelper(matrix [][]int, target int, top, left, bottom, right int) bool {
    // 边界检查
    ...
    fmt.Printf("搜索区域: (%d,%d)→(%d,%d), 中心=(%d,%d)=%d\n",
        top, left, bottom, right, midRow, midCol, midVal)

    if midVal == target {
        return true
    } else if midVal > target {
        // 目标值在左上、右上或左下区域
        return (searchHelper(matrix, target, top, left, midRow, midCol) ||
                searchHelper(matrix, target, top, midCol+1, midRow, right) ||
                searchHelper(matrix, target, midRow+1, left, bottom, midCol))
    } else {
        // 目标值在右上、左下或右下区域
        return (searchHelper(matrix, target, top, midCol+1, midRow, right) ||
                searchHelper(matrix, target, midRow+1, left, bottom, midCol) ||
                searchHelper(matrix, target, midRow+1, midCol+1, bottom, right))
    }
}

// 带调试信息的版本
func searchMatrixWithDebug(matrix [][]int, target int) bool {
    fmt.Printf("=== 搜索二维矩阵 II ===\n")
    fmt.Printf("输入矩阵 (%d×%d):\n", len(matrix), len(matrix[0]))
    printMatrix(matrix)
    fmt.Printf("搜索目标: %d\n", target)
...
| 二分搜索优化 | O(m × log n) | O(1)          | 行有序利用 |
| 分治法       | O(m × n)     | O(log(m × n)) | 理解原理   |
...
func testSearchMatrix(name string, matrix [][]int, target int, expected bool) {
    ...
    // 测试二分搜索优化
    result3 := searchMatrixBinaryOptimized(copyMatrix(matrix), target)
    fmt.Printf("二分搜索优化结果: %t\n", result3)

    // 测试分治法（仅对小矩阵测试）
    var result4 bool
    if len(matrix) <= 10 && len(matrix[0]) <= 10 {
        result4 = searchMatrixDivideConquer(copyMatrix(matrix), target)
        fmt.Printf("分治法结果: %t\n", result4)
    }
    ...
    isValid4 := len(matrix) <= 10 && len(matrix[0]) <= 10 && result4 == expected ||
                len(matrix) > 10 || len(matrix[0]) > 10
    ...
    if isValid1 && isValid2 && isValid3 && isValid4 {
        ...
        if !isValid4 && len(matrix) <= 10 && len(matrix[0]) <= 10 {
            fmt.Printf("  分治法实际: %t\n", result4)
        }
    }
    fmt.Printf("\n")
...
func performanceTest() {
    ...
    // 测试二分搜索优化
    start2 := time.Now()
    searchMatrixBinaryOptimized(copyMatrix(matrix), target)
    time2 := time.Since(start2)

    fmt.Printf("  Z字形搜索: %v\n", time1)
    fmt.Printf("  二分搜索优化: %v\n", time2)
    fmt.Printf("  性能对比: Z字形搜索比二分搜索优化快%.2fx\n", float64(time2)/float64(time1))
}
...
func comparisonTest() {
    ...
    for i := range testData {
        ...
        // 二分搜索优化
        result3 := searchMatrixBinaryOptimized(copyMatrix(testData[i]), testTargets[i])

        fmt.Printf("测试%d: Z字形搜索=%t, 详细实现=%t, 二分搜索优化=%t",
